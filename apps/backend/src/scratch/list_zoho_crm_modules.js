...
async function main() {
  ...
  console.log('\nFetching CRM Modules metadata...');
  const modulesRes = await getZohoRecord(token, '/crm/v2/settings/modules');
  if (modulesRes.data && modulesRes.data.modules) {
    modulesRes.data.modules.forEach(m => {
      console.log(`- API Name: ${m.api_name} | Singular Label: ${m.singular_label} | Plural Label: ${m.plural_label}`);
    });
  }
}
