...
async function main() {
  ...
  console.log('\n--- 1. CREATING BRAND NEW PROJECT SITE ("Final Site V5") ---');
  const projRes = await zohoPost(token, '/crm/v2/Units', { Name: 'Final Site V5' });
  ...
  console.log('\n--- 2. CREATING BRAND NEW UNIT ("Final-Unit-V5") ---');
  const unitRes = await zohoPost(token, '/crm/v2/Products', { Product_Name: 'Final-Unit-V5', Project_Site: projectSiteId });
  const unitId = unitRes.body.data[0].details.id;
  console.log(`Created Unit ID: ${unitId}`);

  // 3. Create brand new Contact
  console.log('\n--- 3. CREATING BRAND NEW CONTACT ("Final Buyer V5") ---');
  const contactRes = await zohoPost(token, '/crm/v2/Contacts', { Last_Name: 'Final Buyer V5', First_Name: 'GoodEarth', Email: 'final.v5@goodearth.org.in' });
  const contactId = contactRes.body.data[0].details.id;
  console.log(`Created Contact ID: ${contactId}`);

  // 4. Create brand new Deal
  console.log('\n--- 4. CREATING BRAND NEW DEAL ("Final-Unit-V5-060826") ---');
  const dealRes = await zohoPost(token, '/crm/v2/Deals', {
    Deal_Name: 'Final-Unit-V5-060826',
    Stage: 'Unit Blocked',
    Contact_Name: contactId,
    Project_Site: projectSiteId,
    Unit_Name: unitId,
    Email: 'final.v5@goodearth.org.in',
  ...
  console.log('\n===============================================================');
  console.log('6. RAILWAY PRODUCTION LOGS FOR FINAL DEAL');
  console.log('===============================================================');
  try {
    const railwayLogs = execSync('railway logs', { encoding: 'utf-8', maxBuffer: 10 * 1024 * 1024 });
    const lines = railwayLogs.split('\n');
    lines.filter(l => l.includes(newDealId) || l.includes('Final Site V5') || l.includes('Final-Unit-V5') || l.includes('Floor Plans') || l.includes('WORKDRIVE_HIERARCHY')).forEach(l => console.log(l));
  } catch (e) {
  ...
  if (rootRes.body && rootRes.body.data) {
    for (const p of rootRes.body.data) {
      if (p.attributes && p.attributes.name === 'Final Site V5') {
